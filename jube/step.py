...
class Operation(object):
    ...
    def add_information_to_database(self, db, step_name):
        ...
        if self._async_filename is not None:
            do_data["async_filename"] = self._async_filename
        if self._error_filename is not None:
            do_data["error_filename"] = self._error_filename
        if self._break_filename is not None:
            do_data["break_filename"] = self._break_filename
        if self._stdout_filename is not None:
            do_data["stdout_filename"] = self._stdout_filename
        if self._stderr_filename is not None:
            do_data["stderr_filename"] = self._stderr_filename
        if self._active != "true":
