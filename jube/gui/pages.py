...
class MainPage(Page):
    ...
    def __init__(self, root, benchmark):
        ...
        self._label_file_path_ref = tk.Label(self,
                                             text="file_path_ref: " +
                                             f"{self._benchmark.file_path_ref!r}",
                                             bg=WHITE)
        self._label_outpath = tk.Label(self,
                                       text="outpath: " +
                                       f"{self._benchmark.outpath!r}",
                                       bg=WHITE)
    ...
    def place_widgets(self):
        ...
        if self._benchmark._comment is not None:
            self._label_comment.config(
                text="comment: "+f"{self._benchmark._comment!r}")
        ...
        if len(self._benchmark._tags) != 0:
            ...
            for tag, desc in self._benchmark._tag_docu.items():
                self._tree_tag_docu.insert(
                    "", "end", text=f"{tag}: {desc}" if tag is not None else "")
        ...
        if len(self._benchmark._tags) != 0:
            ...
            for tag in self._benchmark._tags:
                self._tree_tag.insert(
                    "", "end", text=f"{tag!r}" if tag is not None else "")
    ...
    def create_graph(self):
        ...
        for level in levels.keys():
            ...
            for i in enumerate(levels[level]):
                ...
                column = math.floor(max_columns/2)+2 * \
                    (i[0]-math.floor(length/2))
                if length % 2 == 0:
                    column = column + 1
                ...
                for depend in i[1].depend:
                    coord1 = self._canvas_steps.coords(
                        self._frames_steps[depend])
                    coord2 = self._canvas_steps.coords(
                        self._frames_steps[i[1].name])
    ...
    def configure_scrollregion(self, event):
        ...
        if main_width > canvas_width:
            self._canvas_steps.configure(
                scrollregion=(0, 0, main_width, canvas_height))
            self._scrollbar_x.pack(side="bottom", fill="x")
        else:
            self._canvas_steps.configure(
...
class StepPage(Page):
    ...
    def place_widgets(self):
        ...
        if canvas_bbox:
            ...
            self._parents_canvas.configure(
                yscrollcommand=self._parents_scrollbar.set)
        ...
        if canvas_bbox:
            ...
            self._children_canvas.configure(
                yscrollcommand=self._children_scrollbar.set)
        ...
        self._step_frame.pack(anchor="center", fill="both",
                              side="right", expand="True", pady=25)
        ...
        self._mid_canvas.create_window(
            (0, 0), window=self._mid, anchor="nw", tags="mid")
    ...
    def show_data(self):
        ...
        if self._step.alt_work_dir is not None:
            self._step_tree.insert(
                "", "end", text="work_dir: " + f"{self._step.alt_work_dir!r}")
        if self._step.suffix is not None:
            self._step_tree.insert(
                "", "end", text="suffix: " + f"{self._step.suffix!r}")
        if self._step.shared_link_name is not None:
            self._step_tree.insert(
                "", "end", text="shared: " + f"{self._step.shared_link_name!r}")
        if self._step.active is not None:
            self._step_tree.insert(
                "", "end", text="active: " + f"{self._step.active!r}")
        if self._step.export is not None:
            self._step_tree.insert(
                "", "end", text="export: " + f"{self._step.export!r}")
        if self._step.max_wps is not None:
            self._step_tree.insert(
                "", "end", text="max_async: " + f"{self._step.max_wps!r}")
        if self._step.iterations is not None:
            self._step_tree.insert(
                "", "end", text="iterations: " + f"{self._step.iterations!r}")
        if self._step.cycles is not None:
            self._step_tree.insert(
                "", "end", text="cycles: " + f"{self._step.cycles!r}")
        if self._step.procs is not None:
            self._step_tree.insert(
                "", "end", text="procs: " + f"{self._step.procs!r}")
        if self._step.do_log_file is not None:
            self._step_tree.insert(
                "", "end", text="do_log_file: " + f"{self._step.do_log_file!r}")
    ...
    def configure_scrollregion(self, event=None):
        ...
        parents_height = parent_bbox[3] - \
            parent_bbox[1] if parent_bbox is not None else 0

        children_bbox = self._children_canvas.bbox("all")
        children_height = children_bbox[3] - \
            children_bbox[1] if children_bbox is not None else 0
        ...
        self._parents_canvas.config(
            height=parents_canvas_height, scrollregion=(0, 0, 200, parents_height))
        ...
        if children_height <= window_height:
            ...
            self._children_scrollbar.pack(
                side=tk.RIGHT, fill=tk.Y, pady=25, before=self._children_canvas)
        self._children_canvas.config(
            height=children_canvas_height, scrollregion=(0, 0, 200, children_height))
        ...
        if mid_height >= mid_canvas_height and mid_width >= mid_canvas_width:
            self._mid_canvas.configure(
                scrollregion=(0, 0, mid_width, mid_height))
        elif mid_height >= mid_canvas_height:
            self._mid_canvas.configure(scrollregion=(
                0, 0, mid_canvas_width, mid_height))
        elif mid_width >= mid_canvas_width:
            self._mid_canvas.configure(scrollregion=(
                0, 0, mid_width, mid_canvas_height))
        else:
            self._mid_canvas.configure(scrollregion=(
                0, 0, mid_canvas_width, mid_canvas_height))
...
class WorkpackagePage(Page):
    ...
    def __init__(self, root, wp):
        ...
        self._status = tk.Canvas(self._step_frame_label,
                                 ...
                                 width=22,
                                 height=22)
        self._step_button.config(
            command=lambda: self.step_clicked(self._wp.step.name))
        self._step_tree = ttk.Treeview(self._mid,
                                       selectmode="none",
                                       show="tree",
                                       style="Grey.Custom.Treeview",
    ...
    def place_widgets(self):
        ...
        # pack the widgets for the mid region into the layout
        self._wp_frame.pack(anchor="center", fill="both",
                            side="right", expand="True", pady=25)
        self._mid_scrollbar_y.pack(side=tk.RIGHT, fill=tk.Y)
        self._mid_scrollbar_x.pack(side=tk.BOTTOM, fill=tk.X)
        self._mid_canvas.configure(yscrollcommand=self._mid_scrollbar_y.set)
        ...
        if len(self._wp.env)+len(self._wp.nonenv) != 0:
            ...
            self._env_tree.config(height=len(
                self._wp.env)+len(self._wp.nonenv))
    ...
    def show_data(self):
        ...
        self._step_tree.insert(
            "", "end", text="iteration: " + f"{self._wp.iteration!r}")
        self._step_tree.insert(
            "", "end", text="cycle: " + f"{self._wp.cycle!r}")
