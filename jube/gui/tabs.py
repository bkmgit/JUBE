...
class Tab(tk.Frame):
    ...
    def __init__(self, root, data, app, title):
        ...
        self._window.bind("<B1-ButtonRelease>",
                          lambda e: self.event_generate("<Configure>"))
        ...
        self._canvas.create_window(
            (0, 0), window=self._content, anchor="nw", tags="tab")
    ...
    def configure_scrollregion(self, event=None):
        ...
        if main_width > canvas_width:
            self._canvas.configure(scrollregion=(
                0, 0, main_width, canvas_height))
        else:
            self._canvas.configure(scrollregion=(
                0, 0, canvas_width, canvas_height))
        self._canvas.update_idletasks()
        self._window.config(height=max(
            self._page.winfo_reqheight(), self._menu.winfo_reqheight()))
...
class ParametersetTab(Tab):
    ...
    def __init__(self, root, parametersets, app):
        ...
        if len(self._data) != 0:
            ...
            self.widget_dict[first]["table"].pack(
    ...
    def item_selected(self, event):
        ...
        if item is not None:
            ...
            self.widget_dict[parameterset]["table"].pack(
                fill="both", expand="True", pady=10)
...
class PatternsetTab(Tab):
    ...
    def __init__(self, root, patternsets, app):
        ...
        for patternset in self._data.values():
            ...
            pattern_storage.update(
                patternset.derived_pattern_storage.parameter_dict)
    ...
    def item_selected(self, event):
        ...
        if event.widget == self._listbox and self._listbox.curselection():
            ...
            self.widget_dict[patternset]["table"].pack(
                fill="both", expand="True", pady=10)
            self.configure_scrollregion()
    ...
    def __init__(self, root, filesets, app):
        ...
        if len(self._data) != 0:
            ...
            if self.widget_dict[first]["link_label"]:
                self.widget_dict[first]["link_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[first]["link_tree"]:
                self.widget_dict[first]["link_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[first]["copy_label"]:
                self.widget_dict[first]["copy_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[first]["copy_tree"]:
                self.widget_dict[first]["copy_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[first]["prepare_label"]:
                self.widget_dict[first]["prepare_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[first]["prepare_tree"]:
                self.widget_dict[first]["prepare_tree"].pack(
                    fill="both", pady=(0, 10))
            self._listbox.itemconfig(0, bg=LIGHT_BLUE)
    ...
    def item_selected(self, event):
        ...
        if event.widget == self._listbox and self._listbox.curselection():
            ...
            if self.widget_dict[fileset]["link_label"]:
                self.widget_dict[fileset]["link_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[fileset]["link_tree"]:
                self.widget_dict[fileset]["link_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[fileset]["copy_label"]:
                self.widget_dict[fileset]["copy_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[fileset]["copy_tree"]:
                self.widget_dict[fileset]["copy_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[fileset]["prepare_label"]:
                self.widget_dict[fileset]["prepare_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[fileset]["prepare_tree"]:
                self.widget_dict[fileset]["prepare_tree"].pack(
                    fill="both", pady=(0, 10))
            self.configure_scrollregion()
    ...
    def __init__(self, root, substitutesets, app):
        ...
        if len(self._data) != 0:
            ...
            if self.widget_dict[first]["io_label"]:
                self.widget_dict[first]["io_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[first]["io_tree"]:
                self.widget_dict[first]["io_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[first]["sub_label"]:
                self.widget_dict[first]["sub_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[first]["sub_tree"]:
                self.widget_dict[first]["sub_tree"].pack(
                    fill="both", pady=(0, 10))
            self._listbox.itemconfig(0, bg=LIGHT_BLUE)
    ...
    def item_selected(self, event):
        ...
        if event.widget == self._listbox and self._listbox.curselection():
            ...
            if self.widget_dict[substituteset]["io_label"]:
                self.widget_dict[substituteset]["io_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[substituteset]["io_tree"]:
                self.widget_dict[substituteset]["io_tree"].pack(
                    fill="both", pady=(0, 10))
            if self.widget_dict[substituteset]["sub_label"]:
                self.widget_dict[substituteset]["sub_label"].pack(
                    anchor="w", pady=(10, 0))
            if self.widget_dict[substituteset]["sub_tree"]:
                self.widget_dict[substituteset]["sub_tree"].pack(
                    fill="both", pady=(0, 10))
            self.configure_scrollregion()
...
class AnalyserTab(Tab):
    ...
    def __init__(self, root, analyser, app):
        ...
        for analyser in self._data.values():
            ...
            tree.insert("", "end", text="reduce: " +
                        f"{analyser.reduce_iteration!r}")
    ...
    def __init__(self, root, results, app):
        ...
        for result in self._data.values():
            ...
            if result.result_dir is not None:
                tree_general.insert(
                    "", "end", text="result_dir: " + f"{result.result_dir!r}")
            ...
            if isinstance(result, Table):
                ...
                tree_result.insert(
                    "", "end", text="style: " + f"{result.style!r}")
                if len(result.sort) != 0:
                    tree_result.insert("", "end", text="sort: " +
                                       ", ".join(f"{sort!r}" for sort in result.sort))
                tree_result.insert(
                    "", "end", text="separator: " + f"{result.separator!r}")
                tree_result.insert(
                    "", "end", text="transpose: " + f"{result.transpose!r}")
            ...
            # Create result table for a syslogged result
                ...
                if result.address is not None:
                    tree_result.insert(
                        "", "end", text="address: " + result.address)
                ...
                if result.syslog_string is not None:
                    tree_result.insert(
                        "", "end", text="format: " + result.syslog_string)
                if result.res_filter is not None:
                    tree_result.insert(
                        "", "end", text="filter: " + result.res_filter)
                columns = ["key",
                           "format",
                           "title"]
                table = SortableTreeview(self._content,
            ...
            if isinstance(result, Figure):
                ...
                if result.title is not None:
                    tree_result.insert(
                        "", "end", text="title: " + f"{result.title!r}")
